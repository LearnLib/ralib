...
public class DtlsSerializer {
	...
	public String serializeSymbolInstance(PSymbolInstance symbol) {
		...
		for (DataValue dv : symbol.getParameterValues()) {
			sb.append(dv.getType().getName()).append("=").append(dv.getValue().longValue());
			sb.append(",");
		}
	...
	private DataValue [] parseParamValueStrings(String symbol, String paramString) {
		...
		for (String paramValue : paramValues) {
			...
			BigDecimal concreteValue = valueOf(parts[1], type);
    ...
    public static BigDecimal valueOf(String serializedParameter, DataType parameterType) {
    	return BigDecimal.valueOf(Long.parseLong(serializedParameter));
    }
}
