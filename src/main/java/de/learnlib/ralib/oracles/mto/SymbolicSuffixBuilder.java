...
public class SymbolicSuffixBuilder {
    ...
    public static GeneralizedSymbolicSuffix suffixFromSlice(
            Word<ParameterizedSymbol> actions, Slice slice) {
        
        System.out.println("Suffix from slice: " + slice);
        
        EnumSet<DataRelation>[] prefixRelations = 
                new EnumSet[DataWords.paramLength(actions)];
        ...
        GeneralizedSymbolicSuffix suffix = new GeneralizedSymbolicSuffix(
                actions, prefixRelations, suffixRelations);
    ...
    private static EnumSet<DataRelation> close(EnumSet<DataRelation> set) {
        ...
        if (ret.contains(DataRelation.EQ) || 
                ret.contains(DataRelation.DEQ)) {
            ret.add(DataRelation.EQ);
            ret.add(DataRelation.DEQ);
        }
        
        if (ret.contains(DataRelation.EQ_SUMC1) || 
                ret.contains(DataRelation.DEQ_SUMC1)) {
            ret.add(DataRelation.EQ_SUMC1);
            ret.add(DataRelation.DEQ_SUMC1);
        }
        
        if (ret.contains(DataRelation.EQ_SUMC2) || 
                ret.contains(DataRelation.DEQ_SUMC2)) {
    ...
    public static GeneralizedSymbolicSuffix suffixFromSliceRetainBranching(
            Word<ParameterizedSymbol> acts, Slice slice, SDT sdt) {
