...
public class ThoroughSDTEquivalenceChecker implements de.learnlib.ralib.theory.SDTEquivalenceChecker{
	...
	public ThoroughSDTEquivalenceChecker(Constants constants, ConstraintSolver cSolver) {
		this(constants, cSolver, new Mapping<>());
	}
	
	public ThoroughSDTEquivalenceChecker(Constants constants, ConstraintSolver cSolver, Mapping<SymbolicDataValue, DataValue<?>> guardContext) {
	...
	private boolean areEquivalent(List<Conjunction> path1, SDTGuard guard1, List<Conjunction> path2, SDTGuard guard2) {
		...
		Conjunction disj1UnderGuard2 = new Conjunction(disj1, guard2.toExpr());
		Disjunction disj2 = new Disjunction (path2.toArray(new Conjunction[]{}));
		Conjunction disj2UnderGuard1 = new Conjunction(disj2, guard1.toExpr());
		boolean SAT1 = this.logic.canBothBeSatisfied(new Negation(disj1), new PIV(), disj2UnderGuard1, new PIV(), guardContext);
		boolean SAT2 = this.logic.canBothBeSatisfied(new Negation(disj2), new PIV(), disj1UnderGuard2, new PIV(), guardContext);
		return !SAT1 && !SAT2;
	}
